...
learnLinkageTree(double ** covariance_matrix)
{
  ...
  while (!done)
  {
    ...
    if (NN_chain[NN_chain_length - 1] >= mpm_length)
      NN_chain[NN_chain_length - 1] = mpm_length - 1;

    while (NN_chain_length < 3)
    {
      NN_chain[NN_chain_length] =
        determineNearestNeighbour(NN_chain[NN_chain_length - 1], S_matrix, mpm_number_of_indices, mpm_length);
      NN_chain_length++;
    }
    ...
    if (r1 >= mpm_length || r0 >= mpm_length || mpm_number_of_indices[r0] + mpm_number_of_indices[r1] > FOS_element_ub)
    {
      ...
      if (FOS_element_ub < number_of_parameters)
      {
        ...
        {
          if (mpm_number_of_indices[i] + mpm_number_of_indices[NN_chain[0]] <= FOS_element_ub)
            done = 0;
          if (mpm_number_of_indices[i] < mpm_number_of_indices[NN_chain[0]])
            NN_chain[0] = i;
        }
        if (done)
          break;
    ...
    if (r1 < mpm_length && r1 != r0) /* This test is required for exceptional cases in which the nearest-neighbor
...
{
  if (FOS_element_ub < number_of_parameters && mpm_number_of_indices[a] + mpm_number_of_indices[b] > FOS_element_ub)
    return (0);
  if (random_linkage_tree)
    return (1.0 - fabs(S_vector[a] - S_vector[b]));
  return (S_matrix[a][b]);
}

int
determineNearestNeighbour(int index, double ** S_matrix, int * mpm_number_of_indices, int mpm_length)
{
  int i, result;
  ...
  for (i = 1; i < mpm_length; i++)
  {
    if (((getSimilarity(index, i) > getSimilarity(index, result)) ||
         ((getSimilarity(index, i) == getSimilarity(index, result)) &&
          (mpm_number_of_indices[i] < mpm_number_of_indices[result]))) &&
        (i != index))
      result = i;
  }
...
{
  int i, j, a, b, matches, *permutation, *hungarian_permutation, **FOS_element_similarity_matrix;
